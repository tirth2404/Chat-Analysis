...
if uploaded_file is not None:
    ...
    if df.empty:
        st.error("Could not parse messages from this file. Export chat again with date and time included.")
        st.stop()
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        if df_wc is None:
            st.info("No words available for wordcloud.")
        else:
            fig, ax = plt.subplots()
            ax.imshow(df_wc)
            st.pyplot(fig)
        ...
        st.title('Most Common Words')  # fixed typo: "commmon" -> "Common"
